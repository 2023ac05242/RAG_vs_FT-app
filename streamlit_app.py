...
# Loads data/models from local Out/out OR falls back to GitHub raw URLs
# -------------------------------------------------------
...
# ------------------------------
# Web asset config (GitHub raw URLs) + helpers
# ------------------------------
import os
import streamlit as st
...
def get_setting(key: str, default=None):
    ...
    if v not in (None, ""):
        return v
    try:
        return st.secrets[key]  # works on Streamlit Cloud if you ever add secrets later
    except Exception:
        return default

# Public repo defaults (no secrets needed)
GH_OWNER  = get_setting("GH_OWNER",  "2023ac05242")
GH_REPO   = get_setting("GH_REPO",   "RAG_vs_FT-app")
GH_BRANCH = get_setting("GH_BRANCH", "main")
GH_TOKEN  = get_setting("GH_TOKEN",  None)  # keep but unused for public repos
...
def gh_raw(path: str) -> str:
    ...
    return f"https://raw.githubusercontent.com/{GH_OWNER}/{GH_REPO}/{GH_BRANCH}/{path.lstrip('/')}"

def gh_api_listdir(path: str):
    """List a GitHub repo directory via API; returns items with download_url."""
    url = f"https://api.github.com/repos/{GH_OWNER}/{GH_REPO}/contents/{path.lstrip('/')}?ref={GH_BRANCH}"
    r = requests.get(url, headers=_gh_headers(), timeout=60)
    r.raise_for_status()
    return r.json()

def read_json_local_or_url(path_or_url: str):
    if str(path_or_url).startswith("http"):
        r = requests.get(path_or_url, headers=_gh_headers(), timeout=60)
        r.raise_for_status()
        return json.loads(r.text)
    with open(path_or_url, "r", encoding="utf-8") as f:
        return json.load(f)

def load_torch_state_dict(path_or_url: str, map_location="cpu"):
    # Works for standard torch.save(state_dict) or {"model_state_dict": ...}
    if str(path_or_url).startswith("http"):
        return torch.hub.load_state_dict_from_url(
            path_or_url, map_location=map_location, progress=False
        )
...
# Prefer local; fall back to GitHub raw
FIN_CSV      = in_out("financials_last2y.csv") or gh_raw("out/financials_last2y.csv")
SECTIONS_SRC = in_out("sections_text_yearly")  or "out/sections_text_yearly"  # directory; handled specially
VOCABS_JSON  = in_out("vocabs.json")           or gh_raw("out/vocabs.json")
FT_CKPT      = in_out("minilm_moe_best.pt")    or gh_raw("out/minilm_moe_best.pt")
FT_CKPT_ALT  = in_out("minilm_moe_last.pt")    or gh_raw("out/minilm_moe_last.pt")
CONF_CSV     = in_out("df_extended_view.csv")  or gh_raw("out/df_extended_view.csv")
...
try:
    ...
    st.error(f"Failed to load financials_last2y.csv from {FIN_CSV}: {e}")
    st.stop()
...
ITEM_SCALE = {
    ...
    "operating income ( loss)": 1e9, "net cash from operating activities": 1e9,
    "net cash from investing activities": 1e9, "net cash from financing activities": 1e9,
    "net change in cash": 1e9,
    "depreciation & amortization": 1e6, "research & development": 1e6, "selling, general & administrative": 1e6,
    "accounts & notes receivable": 1e9, "accounts receivable": 1e9, "accounts payable": 1e9,
    "inventories": 1e9, "property, plant & equipment, net": 1e9, "long term investments & receivables": 1e9,
    "total noncurrent assets": 1e9, "total current assets": 1e9, "total noncurrent liabilities": 1e9,
    "total current liabilities": 1e9, "total equity": 1e9, "total liabilities & equity": 1e9,
    "long term debt": 1e9, "short term debt": 1e9, "total debt": 1e9,
    "share capital & additional paid- in capital": 1e9, "retained earnings": 1e9,
    "other comprehensive income": 1e9, "treasury stock": 1e9,
    "basic eps": 1.0, "diluted eps": 1.0, "shares ( basic)": 1.0, "shares ( diluted)": 1.0,
    "income tax ( expense) benefit, net": 1e9, "interest expense, net": 1e9,
    "earnings before tax": 1e9, "income ( loss) from continuing operations": 1e9,
    "net income from discontinued operations": 1e9,
    "dividends paid": 1e9, "cash from ( repurchase of) equity": 1e9, "cash from ( repayment of) debt": 1e9,
    "other operating activities": 1e9, "other investing activities": 1e9, "other financing activities": 1e9,
    "other current assets": 1e9, "other current liabilities": 1e9, "other non-current assets": 1e9,
    "other non-current liabilities": 1e9, "capital expenditures": 1e6,
    "change in fixed assets & intangibles": 1e6, "change in working capital": 1e9,
...
DATASET_METRICS = [
    ...
    "Net Change In Cash","Depreciation & Amortization","Research & Development","Selling, General & Administrative",
    "Accounts & Notes Receivable","Accounts Receivable","Accounts Payable","Inventories","Property, Plant & Equipment, Net",
    "Long Term Investments & Receivables","Total Assets","Total Noncurrent Assets","Total Current Assets","Total Liabilities",
    "Total Noncurrent Liabilities","Total Current Liabilities","Total Equity","Total Liabilities & Equity","Long Term Debt",
    "Short Term Debt","Total Debt","Share Capital & Additional Paid- In Capital","Retained Earnings","Other Comprehensive Income",
    "Treasury Stock","Shares ( Basic)","Shares ( Diluted)","Basic EPS","Diluted EPS","Income Tax ( Expense) Benefit, Net",
    "Interest Expense, Net","Earnings Before Tax","Income ( Loss) From Continuing Operations",
    "Net Income From Discontinued Operations","Dividends Paid","Cash From ( Repurchase Of) Equity",
    "Cash From ( Repayment Of) Debt","Other Operating Activities","Other Investing Activities","Other Financing Activities",
    "Other Current Assets","Other Current Liabilities","Other Non-Current Assets","Other Non-Current Liabilities",
    "Change In Fixed Assets & Intangibles","Capital Expenditures","Change In Working Capital",
    "Cash, Cash Equivalents & Short Term Investments",
]
METRIC_PATTERNS = {k.lower(): [_robust_metric_regex(k)] for k in DATASET_METRICS}
def _add_pats(df_key: str, extra_pats: list):
    lk = df_key.lower(); METRIC_PATTERNS.setdefault(lk, []); METRIC_PATTERNS[lk].extend(extra_pats)
_add_pats("Revenue", [r"^Total\s+Revenue$", r"^Net\s+Sales$", r"^Sales\s+Revenue$", r"^Sales$"])
_add_pats("Cost Of Revenue", [r"^Cost\s+of\s+Goods\s+Sold$", r"^COGS$", r"^Cost\s+of\s+Sales$"])
_add_pats("Gross Profit", [r"^Gross\s+Income$", r"^Gross\s+Profit$"])
_add_pats("Operating Income ( Loss)", [r"^Operating\s+Income$", r"^Operating\s+Profit$", r"^EBIT$", r"^Income\s+from\s+Operations$"])
_add_pats("Net Income", [r"^Net\s+Profit$", r"^Net\s+Earnings$", r"^Profit\s+Attributable$", r"^Earnings$"])
_add_pats("Net Cash From Operating Activities", [r"^Operating\s+Cash\s*Flow$", r"^Cash\s*Flow\s*from\s*Operations$", r"^Net\s*Cash\s*Provided\s*by\s*Operating\s*Activities$"])
_add_pats("Net Cash From Investing Activities", [r"^Investing\s+Cash\s*Flow$", r"^Cash\s*Flow\s*from\s*Investing$"])
_add_pats("Net Cash From Financing Activities", [r"^Financing\s+Cash\s*Flow$", r"^Cash\s*Flow\s*from\s*Financing$"])
_add_pats("Cash, Cash Equivalents & Short Term Investments", [
    r"^Cash(,|\s)+Cash\s+Equivalents\s*(?:&|and)\s*Short[-\s]*Term\s+Investments$",
    r"^Cash\s*(?:&|and)\s*Cash\s+Equivalents$", r"^Cash\s*(?:&|and)\s*Short[-\s]*Term\s+Investments$",
    r"^Cash\s*(?:&|and)\s*Equivalents$",
])
_add_pats("Total Equity", [
    r"^Total\s+(?:Share|Stock)holders'?\s+Equity$", r"^Shareholders'?\s+Equity$", r"^Stockholders'?\s+Equity$",
    r"^Total\s+Shareholders'?\s+Funds$",
])
_add_pats("Capital Expenditures", [
    r"^Capital\s+Expenditures?$", r"^Purchase[s]?\s+of\s+Property,?\s+Plant\s*(?:&|and)\s*Equipment$",
    r"^Additions\s+to\s+Property,?\s+Plant\s*(?:&|and)\s*Equipment$", r"^Purchases\s+of\s+PP&E$",
])
_add_pats("Long Term Debt", [r"^Long[-\s]*Term\s+Debt$", r"^Noncurrent\s+Debt$"])
_add_pats("Short Term Debt", [r"^Short[-\s]*Term\s+Debt$", r"^Current\s+Portion\s+of\s+Debt$"])
_add_pats("Total Liabilities & Equity", [r"^Total\s+Liabilities\s*(?:&|and)\s+Equity$", r"^Liabilities\s*(?:&|and)\s+(?:Share|Stock)holders'?\s+Equity$"])
_add_pats("Share Capital & Additional Paid- In Capital", [r"^Additional\s+Paid[-\s]*In\s+Capital$", r"^Share\s+Premium$"])
_add_pats("Earnings Before Tax", [r"^Earnings\s+Before\s+Tax(?:es)?$", r"^Income\s+Before\s+Tax(?:es)?$", r"^EBT$"])
_add_pats("Income Tax ( Expense) Benefit, Net", [r"^Provision\s+for\s+Income\s+Taxes$", r"^Income\s+Tax\s+Expense$"])
_add_pats("Interest Expense, Net", [r"^Interest\s+Expense(?:,\s*Net)?$"])
_add_pats("Cash From ( Repurchase Of) Equity", [r"^Share\s+Repurchases$", r"^Stock\s+Buybacks$", r"^Repurchase\s+of\s+Common\s+Stock$"])
_add_pats("Cash From ( Repayment Of) Debt", [r"^Issuance\s*\(Repayment\)\s+of\s+Debt$", r"^Net\s+Borrowings$"])

METRIC_SYNONYMS = {
    "revenue": ["revenue","total revenue","net sales","sales revenue","sales","turnover"],
    "cost of revenue": ["cost of revenue","cost of goods sold","cogs","cost of sales"],
    "gross profit": ["gross profit","gross income","gross margin (dollars)","gross margin dollars"],
    "operating income ( loss)": ["operating income","operating profit","income from operations","ebit"],
    "net income": ["net income","net profit","net earnings","profit attributable","earnings"],
    "depreciation & amortization": ["depreciation and amortization","d&a","depr & amort"],
    "research & development": ["research and development","r&d expense","r and d"],
    "selling, general & administrative": ["selling general and administrative","selling general & administrative","sga","sg&a"],
    "net cash from operating activities": ["operating cash flow","cash flow from operations","net cash provided by operating activities","operating cashflow"],
    "net cash from investing activities": ["investing cash flow","cash flow from investing"],
    "net cash from financing activities": ["financing cash flow","cash flow from financing"],
    ...
    "accounts & notes receivable": ["accounts receivable","notes receivable","trade receivables","receivables"],
    "accounts receivable": ["accounts receivable","trade receivables","receivables"],
    "accounts payable": ["accounts payable","trade payables","payables"],
    "inventories": ["inventory","inventories - net"],
    "property, plant & equipment, net": ["pp&e net","property plant and equipment net","ppe net","property plant and equipment"],
    "long term investments & receivables": ["long-term investments","long term investments","noncurrent investments"],
    "total assets": ["total assets"], "total noncurrent assets": ["noncurrent assets","long term assets","long-term assets"],
    "total current assets": ["current assets","total current assets"], "total liabilities": ["total liabilities"],
    "total noncurrent liabilities": ["noncurrent liabilities","long term liabilities","long-term liabilities"],
    "total current liabilities": ["current liabilities","total current liabilities"],
    "total equity": ["total equity","shareholders equity","stockholders equity","total shareholders equity","total stockholders equity","common equity","equity (balance sheet)"],
    "total liabilities & equity": ["total liabilities and equity","liabilities and shareholders equity","liabilities and stockholders equity"],
    "long term debt": ["long term debt","long-term debt","lt debt","noncurrent debt"],
    "short term debt": ["short term debt","short-term debt","st debt","current portion of debt"],
    "total debt": ["total debt","total borrowings","gross debt","interest bearing debt"],
    "share capital & additional paid- in capital": ["additional paid in capital","apic","share premium","share capital and apic"],
    "retained earnings": ["retained earnings","accumulated earnings","accumulated deficit"],
    "other comprehensive income": ["accumulated other comprehensive income","aoci","other comprehensive income"],
    "treasury stock": ["treasury stock","treasury shares","treasury stock at cost"],
    "shares ( basic)": ["basic shares outstanding","weighted average shares basic","basic shares"],
    "shares ( diluted)": ["diluted shares outstanding","weighted average shares diluted","diluted shares"],
    "basic eps": ["earnings per share basic","basic earnings per share","eps basic"],
    "diluted eps": ["earnings per share diluted","diluted earnings per share","eps diluted"],
    "income tax ( expense) benefit, net": ["income taxes","income tax expense","provision for income taxes"],
    "interest expense, net": ["interest expense net","interest expense"],
    "earnings before tax": ["earnings before tax","income before tax","ebt"],
    "income ( loss) from continuing operations": ["income from continuing operations","loss from continuing operations"],
    "net income from discontinued operations": ["income from discontinued operations","discontinued operations income"],
    "dividends paid": ["dividends paid","cash dividends"],
    "cash from ( repurchase of) equity": ["share repurchases","stock buybacks","repurchase of common stock","buyback of shares"],
    "cash from ( repayment of) debt": ["debt issued repaid","issuance (repayment) of debt","net borrowings","debt repayments"],
    "other operating activities": ["other operating cash flows"],
    "other investing activities": ["other investing cash flows"],
    "other financing activities": ["other financing cash flows"],
    "other current assets": ["other current assets"], "other current liabilities": ["other current liabilities"],
    "other non-current assets": ["other noncurrent assets"], "other non-current liabilities": ["other noncurrent liabilities"],
    "change in fixed assets & intangibles": ["capex","capital expenditures","investment in ppe","purchase of ppe","change in fixed assets and intangibles"],
    "capital expenditures": ["capex","capital expenditures","purchases of property plant and equipment","additions to property plant and equipment","purchases of ppe"],
    "change in working capital": ["change in working capital","working capital change"],
}
...
def parse_query(q: str) -> Tuple[Optional[str], Optional[int]]:
    ...
    for t in SUPPORTED_TICKERS:
        if re.search(rf"\b{re.escape(t)}\b", q): ticker = t; break
    if not ticker:
        ql = q.lower()
        for alias, tk in NAME_ALIASES.items():
            if ((len(alias) <= 6 and re.search(rf"\b{re.escape(alias)}\b", ql)) or (len(alias) > 6 and alias in ql)):
                if tk in SUPPORTED_TICKERS:
                    ticker = tk; break
    m = re.search(r"\b(20\d{2})\b", q); year = int(m.group(1)) if m else None
    return ticker, year
...
def _canonicalize_entities_for_match(text: str) -> str:
    ...
    for alias, tk in sorted(NAME_ALIASES.items(), key=lambda kv: -len(kv[0])):
        al = alias.lower()
        if len(al) <= 6: t = re.sub(rf"\b{re.escape(al)}\b", tk.lower(), t)
        else: t = t.replace(al, tk.lower())
...
def _normalize_question(text: str) -> str:
    ...
    t = unicodedata.normalize("NFKD", text).replace("’","'").replace("“","\"").replace("”","\"").lower()
    t = re.sub(r"[^a-z0-9\s$%./-]+", " ", t)
    t = re.sub(r"\s+", " ", t).strip()
    return t
...
def _nl_similarity(a: str, b: str) -> float:
    a_n = _normalize_question(a); b_n = _normalize_question(b)
    if not a_n and not b_n: return 0.0
...
def _norm_method(m):
    if isinstance(m, pd.Series):
        ...
        s = s.replace({"ft":"finetuned","finetune":"finetuned","finetuned":"finetuned","finetuning":"finetuned",
                       "finetunedmodel":"finetuned","finetunedmethod":"finetuned","finetunemethod":"finetuned",
                       "finetunedmodelmethod":"finetuned"})
        return s
    else:
        m = "" if m is None else str(m)
        m = re.sub(r"[\s\-_]+", "", m.lower().strip())
        return "finetuned" if m in {"ft","finetune","finetuned","finetuning","finetunedmodel","finetunedmethod","finetunemethod","finetunedmodelmethod"} else m

def _extract_struct(q: str) -> Tuple[Optional[str], Optional[int], Optional[str]]:
    tkr, yr = parse_query(q); met = detect_metric(q)
    return tkr, yr, met

def confidence_from_csv(question: str, method_label: str, tol: float = 0.80) -> Optional[float]:
    dfc = load_conf_table(CONF_CSV)
    if dfc is None or dfc.empty: return None
    ...
    for idx, row in cand_df.iterrows():
        cand_q = row["Question"]; cand_norm = _normalize_question(cand_q)
        if cand_norm == q_norm:
            try: return float(row["Confidence"])
        ...
        bonus = (0.08 if struct_matches==1 else 0.16 if struct_matches==2 else 0.24 if struct_matches==3 else 0.0)
        if _norm_method(row["Method"]) == want_method: bonus += 0.03
        score = s_base + bonus
        if score > best_score:
            best_score = score
            try: best_conf = float(row["Confidence"]); best_idx = idx
            except Exception: best_conf = None; best_idx = idx
    if best_conf is not None and best_score >= tol: return best_conf
    if best_conf is not None:
        cand_q_best = cand_df.loc[best_idx, "Question"]
        user_tokens = _token_set(q_norm); cand_tokens = _token_set(_normalize_question(cand_q_best))
        jacc = (len(user_tokens & cand_tokens) / max(1, len(user_tokens | cand_tokens))) if (user_tokens or cand_tokens) else 0.0
        ct, cy, cm = _extract_struct(cand_q_best); struct_overlap = 0
        ...
        if struct_overlap >= 2 and best_score >= 0.60: return best_conf
        if best_score >= 0.74 and (struct_overlap >= 1 or jacc >= 0.60): return best_conf
    return None
...
def load_sections(src: str) -> List[Dict]:
    """
    Load 'sections_text_yearly/*.txt' either from a local directory (if exists)
    or from GitHub (using the Contents API to list and then raw URLs to fetch).
    """
    docs = []
    ...
    # GitHub mode — list directory via API then fetch each file via download_url
    try:
        items = gh_api_listdir(src)
        for it in items:
            if it.get("type") != "file" or not it.get("name", "").endswith(".txt"):
                continue
            base = it["name"]
            if base.count("__") != 2:
                continue
            ticker, statement, tail = base.split("__")
            year = int(tail.rsplit(".", 1)[0])
            r = requests.get(it["download_url"], headers=_gh_headers(), timeout=60)
            r.raise_for_status()
            text = r.text
            docs.append({"ticker": ticker, "statement": statement, "year": year, "path": it["download_url"], "text": text})
    except Exception as e:
        st.warning(f"Could not load sections from GitHub at '{src}': {e}")
...
sections, texts, Xdense, faiss_index, sparse_pack = build_rag_indexes(SECTIONS_SRC)
...
def citations_from_contexts(ctxs, limit: int = 3):
    ...
    for c in ctxs:
        ...
        seen.add(key)
        base = os.path.basename(c["path"]) if not str(c["path"]).startswith("http") else os.path.basename(str(c["path"]))
        cites.append(base)
...
def answer_rag_pipeline(q: str, k_ctx:int=5, use_ce:bool=True) -> dict:
    if _has_out_of_scope_mention(q) or not _looks_financial(q):
        ...
        return {"result": {"answer": msg, "confidence": 0.10, "ticker": None, "year": None, "metric": None, "value_norm": None}, "contexts": []}
    ...
    if direct:
        ...
        return {"result": {"answer": ans_text, "confidence": float(confidence), "ticker": tkr, "year": yr, "metric": metric, "value_norm": float(direct["value_norm"])}, "contexts": ctxs}
    bullets = []
    ...
    return {"result": {"answer": ans, "confidence": float(confidence), "ticker": tkr, "year": yr, "metric": metric}, "contexts": ctxs}
...
def load_ft_components():
    ...
    voc = read_json_local_or_url(VOCABS_JSON)
    ...
    class MiniLM_MoE_Regressor(torch.nn.Module):
        ...
        def forward(self, texts, t_idx, k_idx, y_idx, max_len=128, device="cpu"):
            ...
            pooled = self._mean_pool(out.last_hidden_state, enc_in["attention_mask"])
            ...
            h_cat = torch.cat([pooled, et, ek, ey], dim=-1)
            want = getattr(self.proj, "in_features", h_cat.shape[-1]); have = h_cat.shape[-1]
            if have != want:
                if have < want:
                    pad = want - have
    ...
    try:
        state = load_torch_state_dict(FT_CKPT, map_location=dev)
    except Exception:
        state = load_torch_state_dict(FT_CKPT_ALT, map_location=dev)
    state = state.get("model_state_dict", state)
    try:
        model.load_state_dict(state, strict=True)
    except RuntimeError:
        model.load_state_dict(state, strict=False)
        st.warning("Loaded FT checkpoint non-strictly (shapes differed). Ensure checkpoint matches encoder + side-embedding dims.")
    model.eval()
...
def predict_usd_from_row_embY(row: dict) -> float:
    ...
    dev    = FT["device"]; moe_z  = FT["moe_z"]; ZM, ZS = FT["Z_MEAN"], FT["Z_STD"]; MAXLEN = FT["MAX_LEN"]
    ...
    y_raw = row.get("year"); y_idx = torch.tensor([y2i.get(int(y_raw), 0) if isinstance(y_raw, int) else 0], dtype=torch.long, device=dev)
...
def answer_ft(ticker: Optional[str], year: Optional[int], metric: Optional[str], raw_query: str = "", use_moe: bool = True) -> dict:
    ...
    if offenders:
        return {"answer": _fallback_out_of_scope_message(offenders), "y_hat": None, "confidence": 0.10, "secs": 0.0}
    if not _looks_financial(raw_query):
        return {"answer": _fallback_out_of_scope_message(None), "y_hat": None, "confidence": 0.10, "secs": 0.0}
    if not ticker or ticker not in SUPPORTED_TICKERS:
        return {"answer": _fallback_out_of_scope_message(None), "y_hat": None, "confidence": 0.10, "secs": 0.0}
    metric_l = (metric or detect_metric(raw_query) or "").strip().lower()
    if not metric_l:
        return {"answer": _fallback_unknown_metric_message(ticker, year), "y_hat": None, "confidence": 0.20, "secs": 0.0}
    scale = ITEM_SCALE.get(metric_l, 1.0)
    if not FT:
        return {"answer": "FT model not available", "y_hat": None, "confidence": 0.0, "secs": 0.0}
    try:
    ...
    row = {"q": q_aug, "ticker": ticker, "year": int(year) if isinstance(year, int) else None, "canon_key": metric_l, "scale": scale}
    t0 = time.time()
    try:
        y_hat = predict_usd_from_row_embY(row); secs = time.time() - t0; confidence = ft_confidence(row, y_hat)
        return {"answer": fmt_money(y_hat), "y_hat": y_hat, "confidence": confidence, "secs": round(secs, 3)}
    except RuntimeError:
        secs = time.time() - t0
        msg = ("Fine-tuned model shape mismatch. Ensure the checkpoint matches the current architecture "
               "(encoder hidden size and embedding dims for ticker/key/year), or disable 'FT: Mixture-of-Experts' in Controls.")
...
# --- Out-of-scope & finance-intent helpers ---
...
_CAPS_IGNORE = {"USD","EUR","GBP","EPS","EBITDA","GAAP","CAGR","FY","Q1","Q2","Q3","Q4","CEO","CFO","PPE","COGS","SGA","DCF","WACC","FCF","OCF","BS","CF","IS","JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"}
...
def _looks_financial(text: str) -> bool:
    ...
    tl = text.lower()
    if detect_metric(text) is not None: return True
    if any(re.search(rf"\b{re.escape(t)}\b", text) for t in SUPPORTED_TICKERS): return True
    if any(p in tl for p in FINANCE_HINT_PHRASES): return True
    if any(re.search(rf"\b{re.escape(w)}\b", tl) for w in FINANCE_HINT_WORDS): return True
    return False
...
def _fallback_unknown_metric_message(tkr: Optional[str], yr: Optional[int]) -> str:
    ...
    return f"I couldn’t recognize a financial metric in your question for {who}{when}.\n\nTry one of these: {shown}"
...
method = st.sidebar.radio("Method", ["RAG"] + (["FT"] if FT else []), index=0, key="method")
st.sidebar.checkbox("RAG: Cross encoder rank", disabled=(method != "RAG"), key="rag_use_ce")
st.sidebar.checkbox("FT: Advanced technique — Mixture-of-Experts", disabled=(method != "FT" or not FT), key="ft_use_moe")
...
with colM:
    ...
    q = st.text_area(label="", placeholder="Type your question here", key="q_input", label_visibility="collapsed", height=1)
    go = st.button("Answer", type="primary")

    if go:
        oos_mentions = _find_out_of_scope_mentions(q)
        if oos_mentions:
            ar = {"method": method, "answer": _fallback_out_of_scope_message(oos_mentions), "confidence": 0.10, "time_total": 0.0,
                  "ticker": None, "year": None, "metric": None, "citations": [], "correct": False}
        elif not _looks_financial(q):
            ar = {"method": method, "answer": _fallback_out_of_scope_message(), "confidence": 0.10, "time_total": 0.0,
                  "ticker": None, "year": None, "metric": None, "citations": [], "correct": False}
        else:
            tkr_pre, yr_pre = parse_query(q); met_pre = detect_metric(q)
            if (tkr_pre or yr_pre) and met_pre is None:
                ar = {"method": method, "answer": _fallback_unknown_metric_message(tkr_pre, yr_pre), "confidence": 0.20,
                      "time_total": 0.0, "ticker": tkr_pre, "year": yr_pre, "metric": None, "citations": [], "correct": False}
            else:
                if method == "RAG":
                    t0 = time.time()
                    ...
                    correct = bool(gt is not None and pred is not None and abs((pred or 0) - gt)/denom <= 0.02)
                    ar = {"method":"RAG","answer": r["answer"],"confidence": float(r["confidence"]),
                          ...
                          "correct": correct, "has_numeric": has_numeric}
                else:
                    tkr, yr = parse_query(q)
                    ...
                    if not FT:
                        ...
                        ar = {"method":"FT","answer": out["answer"],"confidence": float(out["confidence"]),
                              "time_total": float(out["secs"]), "ticker": tkr, "year": yr, "metric": met,
                              "citations": [], "correct": bool(correct), "has_numeric": out.get("y_hat") is not None}
...
with colR:
    ...
    if ss["rows"]:
        ...
        if ss.get("show_table", False):
            st.dataframe(dfres, use_container_width=True, height=min(420, 42 + 24 * min(len(dfres), 12)))
        st.download_button("Export Session Results (CSV)", data=dfres.to_csv(index=False).encode(), file_name="session_results.csv", mime="text/csv")
        st.download_button("Export Session Results (HTML)", data=dfres.to_html(index=False), file_name="session_results.html", mime="text/html")
    else:
        if ss.get("show_table", False):
    ...
    def run_save_script_and_load():
        ...
        p1 = in_out("df_extended_view.csv") or gh_raw("out/df_extended_view.csv")
        p2 = in_out("summary_42.csv")       or gh_raw("out/summary_42.csv")
        p3 = in_out("pair_42.csv")          or gh_raw("out/pair_42.csv")
