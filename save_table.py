...
# Optional helpers for PDF/HTML OCR/extraction (only if you plan to parse your own docs)
# %pip install -q pdfplumber beautifulsoup4 html5lib pytesseract pillow
...
# # 3.5 — MiniLM + MoE (Advanced FT Technique: Mixture-of-Experts)
...
# # Partially unfreeze last MiniLM block
# last_layer_params = []
...
# for p in moe_head_params: p.requires_grad = True

# param_groups = [
#     {"params": moe_head_params,   "lr": 2e-3, "weight_decay": 0.01},
#     {"params": last_layer_params, "lr": 2e-4, "weight_decay": 0.01},
# ]
# opt = torch.optim.AdamW(param_groups)

# # Train / Eval loop + early stopping (z-space only)
...
#         loss = nn.SmoothL1Loss()(out["z_pred"], z_norm)
#         if train:
#             loss.backward()
...
#     t0 = time.time()
#     tr = run_epoch_norm(moe_z, train_loader, train=True)
#     vl = run_epoch_norm(moe_z, val_loader,   train=False)
#     dt = time.time() - t0
...
#         best_ep  = ep
#         bad      = 0
#         save_ckpt_best(moe_z, CKPT_BEST, ep, vl)
#     else:
#         bad += 1
...
# (optional) save
to_report(df_official).to_csv("out/df_official.csv", index=False)


# ===== 4.1 summary =====
summary_41 = compare_speed_accuracy(df_official, include_irrelevant=False)
summary_41.to_csv("out/summary_41.csv", index=False)
...
df_extended_view.to_csv("out/df_extended_view.csv", index=False)


# ===== 4.2 summary =====
summary_42 = compare_speed_accuracy(df_extended, include_irrelevant=False)
summary_42.to_csv("out/summary_42.csv", index=False)

pair_42 = pairwise_table(summary_42)
if pair_42 is not None:
    pair_42.to_csv("out/pair_42.csv", index=False)
